...
Again I prepare empty matrices, but I don't know how many SNPs of each type I'll get:
...
You can get basic information on your data using summaries, which returns both a visible output and an invisible list:
...
You can also look for population structure with F-statistics. You can calculate Fst (pop/total), Fit (ind/total), and Fis (ind/pop) by locus.

First we need to read in data with only two alleles.

```{r}
##Find file with only diploid data?
```

Then we can calcute the F statistics for a single population.

```{r}
Fst(as.loci(NA_mean[NA_mean$pop=="T1_11",c(7,11,18)]))

data("nancycats")
Fst(as.loci(nancycats))
pairwise.fst(nancycats[1:50,])
Fst(as.loci(NA_mean[1:10, 1:10]))


obj2<-obj1[pop=c("T1_11", "T1_12"), c(1,11,18)]
Fst(as.loci(obj2))
...
```{r}
temp <- inbreeding(obj1[obj1$pop=="T1_11",1:100], N=100)

```

It complaines a little bit, but the result should be fine.

```{r}
knitr::kable(temp$`1480_S104`[1:10])
```
