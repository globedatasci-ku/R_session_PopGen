...
---
Install the following packages.{>>I'm not sure this is needed Maybe just devide the loading of the libraries to several chunks and write it above each chunk<<}

For the sequence data manipulation and missingness plotting:{>>Like this<<}
```{r}
library(seqinr)
library(gplots)
...
```
{>>I think we can add the installation instruction for this package in the email we send out. This way we also will have time to deal with installation problems if there will be any<<}
```{r}
...
Below you see an example where I used a aligned fasta file, checked whether the triplicated       samples (library in triplicate, treated as separate samples during assembly) had differences in   their sequence between replicates of individuals, or whether differences could be attritbuted     to missing data.{>>This sentence is a bit complicated<<}
...
#@@#@ There was a naming mistake. Is this doing now what you wanted it to do?
...
It is nice to be able to filter your data manually. This allows you to understand better what you are working with. Keep in mind that with large SNP data this is probably not possible. 

Examples of SNPs that you might want to filter out are singletons{>>Explaine here oraly what is singltone data in a few words<<}. Or you might want to work with only the di-allelic SNPs in your dataset, because many softwares only utilise di-allelic SNPs. In my case, I wanted to see which SNPs were present as one variant in one of my species, and as another in another species. With these SNPs I would be able to analyse the hybrids between both species. In the next code, I filtered my data for such 'species specific' SNPs based on my assumed 'pure' populations.

##NOTE: add a map at this point for checking the populations?{>>Map of what?<<}

{>>Just a general note, I myself prefer smaller chucks and put the comments into the regular text. I think it makes reading the html easier. Your choice<<}
```{r}
...
Not_diagnostic_old<-matrix(NA,nrow=114, ncol=0)
Diagnostic_old<-matrix(NA,nrow=114,ncol=0)
Not_diagnostic<-matrix(NA,nrow=114, ncol=0)
Diagnostic<-matrix(NA,nrow=114,ncol=0)
...
#Make the data structure appropriate for PopHelper
...
#Make the population labels appropriate for PopHelper
...
#@#@ I added the numbers in the code so that it would knit into html
...
## Hardy Weinberg Equilibrium {>>What are we calculating here?<<}
```{r}
